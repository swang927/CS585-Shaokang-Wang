// red_guard_controller.h
    
#ifndef INCLUDED_RED_GUARD_CONTROLLER
#define INCLUDED_RED_GUARD_CONTROLLER
...
namespace mgc
{
// AI controller for red guard
class RedGuardController{
  private:
 ...
 public:
	...
	RedGuardController(sf::RenderWindow* window, sgds::NxNSceneGraph* grid, mgc::StateMachine* state_machine);
	// Operator overload
	const RedGuardController& operator=(const RedGuardController &other);
	// Deconstructor
	~RedGuardController();
...
// constructor
inline
RedGuardController::RedGuardController(){
}

// creat controller with relate window and grid
inline
RedGuardController::RedGuardController(sf::RenderWindow* window, sgds::NxNSceneGraph* grid, mgc::StateMachine* state_machine){
	d_window = window;
	d_grid = grid;
...
// Operator overload
inline
const RedGuardController& RedGuardController::operator = (const RedGuardController &other){
	if (this != &other){
...
inline
void RedGuardController::add_instance(sgda::Guard * guard){
	assert(guard);
	if (has_init)
		init(guard);
	guards.push(guard);
...
inline
void RedGuardController::remove_instance(sgda::Guard * guard){
	assert(guard);
	guards.remove(guard);
	num--;
};
...
// initial certain object data
inline
void RedGuardController::init(sgda::Guard* guard){
	// direction
...
void RedGuardController::init(){
	...
	for (unsigned int i = 0; i < num; i++){
		current.push(d_state_machine->states[28]);
		previous.push(d_state_machine->states[28]);
		// start position
...
inline
void RedGuardController::update(){
	for (unsigned int i = 0; i < num; i++){
		if (guards[i]->isOff()){
...
// updata current direction
inline
void RedGuardController::updata_direction(sgda::Guard* guard, int i){
	//  x
	guard->dirction_x = 0;
	if (next[i]->d_bound.x() - current[i]->d_bound.x() > 0)
		guard->dirction_x = 1;
...
// move actor on x axis
inline
void RedGuardController::move_X(sgda::Guard* guard, int index){
	...
	if (block){
		...
		if (block == d_state_machine->blue && next[index] != current[index]){
...
// move actor on y axis
inline
void RedGuardController::move_Y(sgda::Guard* guard, int index){
	const sgds::ICollider* block;
	// move on x axis
...
// check the collision
inline
const sgds::ICollider* RedGuardController::collisionWith(sgds::ICollider* self){
...
// move from cell to cell
inline
void RedGuardController::moveCell(sgda::Guard* guard){
	// check if the actor move to different cell; if did swith the cell
	int place = d_grid->getCellindex(guard->d_bound);
	if (place != guard->getCell()){
		d_grid->removeCollider(guard, guard->getCell());
...
// set model 
inline
void RedGuardController::setModel(int i){
	model = i;
}

// get model
inline
int RedGuardController::getModel(){
	return model;
}
